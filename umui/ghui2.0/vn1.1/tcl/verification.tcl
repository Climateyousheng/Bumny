...
proc check_variable_value {variable value type size flag} {
	...
	if {$vi_expr!=1} { 
		...
		if {$failed==0} {
			...
			# called with $check_function $value $variable [ expr $i + $start_index] 
                        if {$check_function_args != "" } {
                           set rv [ $check_function $value $variable [ expr $i + $start_index] $check_function_args]
                        } else {
                           set rv [ $check_function $value $variable [ expr $i + $start_index] ]
                        }
			if { $rv != 0} {
			    set failed 1
			}
	...
	if {$inactive!=1} { 
		...
		if {[check_string $variable $value $var_index]!=0} {
		    return 1
                ...
                if {  $check_function_args != "" } { 
                  set rv [ $check_function $value $variable -1 $check_function_args] 
                } else {
                  set rv [ $check_function $value $variable -1 ]
                }
		if { $rv != 0 } {
		    return 1
		}
