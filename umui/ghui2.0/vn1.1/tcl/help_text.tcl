...
proc general_help_text {win_text line_no} {
    # Returns a string containing general help for a variable
    # ie just text for .check .entry .basrad
    # but table name and column name for .entry
    #
    # win_text should be a list of lines from a window panel.
    ...
    if { $type==".entry" } {return "Entry box: [lindex $line 1]"}
    if { $type==".check" } {return "Check box: [lindex $line 1]"}
    if { $type==".basrad" } {return "Radio button: [lindex $line 1]"}
    if { $type==".set_on_closure" } {return "[lindex $line 1]"}

    if { $type==".element" } {return "Table"}
