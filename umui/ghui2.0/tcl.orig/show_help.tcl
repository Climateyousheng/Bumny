...
proc window_help {parent_win} {
    set file_name [string range [toplev_name $parent_win] 1 end]
...
proc show_help {file_name title} {
    set help_file [ghui_help_file $file_name]
    set path [ghuidir_path help]
...
proc include_help_text {text path} {
	...
	set text $output
	if {[llength $text]>500} {
	    multioption_dialog .help {Too much help} "More than 500 lines of help is not allowed. Possible recursion of include files? Please check and reduce" {OK}
	    set include 0
	}
    }
    return $text
