...
# Set up any defaults that are not essential in application definition files
    set_default_globals
    ...
    # Read in pathnames to application and GHUI first
    ...
    # Then read in all other information
    while {[gets $a line] != -1} {
	switch [lindex $line 0] {
	    ...
	    Dimensions {
		set_global_variables $a $app Dimensions
	    }
	    ExpInitial {
		getExpInitialDetails $a $app
	    }
	}
    }
    close $a
    ...
    # Adddition of privacy column
    foreach col {id version description access_list privacy opened owner} {
...
proc set_global_variables {a app list} {
    # Global variables for dimensions of entry window
    global relative_width line_height number_of_lines port
	...
	switch $variable {
	    RELATIVE_WIDTH {
		set relative_width $value
	    }
	    LINE_HEIGHT {
		set line_height $value
	    }
	    NUMBER_OF_LINES {
		set number_of_lines $value
	    }
	    port {
		set port $value
	    }
...
proc set_default_globals {} {
    global relative_width line_height number_of_lines
    global expIdInfo

# Default values
    set relative_width .7
    set line_height 32
    set number_of_lines 20
    set expIdInfo(text) [list \
	    "Select initial letter for newly created experiments"]
}
