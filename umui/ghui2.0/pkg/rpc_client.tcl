...
proc start_rpc_client {server_name server_port server_type} {
    ...
    #ILP
    set str1 [info nameofexecutable]
    set str2 [file tail $str1]
    set rc [string first "wish" $str2]
    if {$rc == 0} {
        set exe_name "wish"
    } else {
        set exe_name "tclsh"
    ...
    if [catch {set server [MakeRPCClient $server_name $server_port]} errmsg] {
	    ...
	    if {$RPCErrorFlag == 1} {
            error "Command cancelled following server error. \
    ...
    } elseif {$exe_name== "tclsh"} {

        if [catch {set server [MakeRPCClient $server_name $server_port]} errmsg] {

	        if {$server_type == "PRIMARY"} {
                set server "NONE"
                puts "Could not connect to the $server_type server ($server_name).\
                    \nThe $server_type server ($server_name) may be down.\
                    \nThe $server_type server ($server_name) may not be running.\
                    \nThere may be a network failure.\
                    \nPLEASE report to Server Administrators about the problem.\
                    \n \nError: $errmsg"
	        ...
	        if {$server_status != "ACTIVE"} {
                ...
                puts "The $server_type server ($server_name) is running normally,\
                    \nbut is $server_status. It is best to Quit if this occurs.\
                    \n\nContact Server administrators if you require more information."
                exit
	        }
        }
        return $server  
...
proc rpc_client_closed {} {
    ...
    tclsh {
    # new code - when running via script 
    
        puts "DATABASE $server_type SERVER CONNECTION CLOSED \
	        \nThe machine $server_name may be down. \
            \nThe $server_type server may have been killed.\
	        \nThere may be a network failure."
        exit
    ...
    if {$rc == 0} {
        ...
        switch $exe_name {
            ...
            # new code - when running via script 
                ...
                puts "The operation failed because the $type server\
		            \nhas been reconfigured, by the admin team, to be $server_type.\
		            \nYou should Quit now and Restart the client\"
		        exit
            }
        }
	    return 1
