...
proc navigation {} {
    ...
    # Must define y coord of canvases in terms of pixels because values are used by scrolling
    # routines for testing whether edge of canvas has been reached
    #canvas $c -scrollregion {0c 0c 15c 1500} -height 12c -width 7c \
	    -yscrollcommand "$f2.vscrol1 set" -xscrollcommand "$f2.hscrol1 set" -yscrollincrement $c_width
    #canvas $d -scrollregion {0c 0c 15c 1500} -height 12c -width 7c \
            -yscrollcommand "$f3.vscrol2 set" -xscrollcommand "$f3.hscrol2 set" -yscrollincrement $c_width
    canvas $c -scrollregion {0 0 15 1500} \
	    -yscrollcommand "$f2.vscrol1 set" -xscrollcommand "$f2.hscrol1 set" -yscrollincrement $c_width
    canvas $d -scrollregion {0 0 15 1500}  \
            -yscrollcommand "$f3.vscrol2 set" -xscrollcommand "$f3.hscrol2 set" -yscrollincrement $c_width
    scrollbar $f2.vscrol1  -relief sunken -command "$c yview"
    scrollbar $f3.vscrol2  -relief sunken -command "$d yview"
    scrollbar $f2.hscrol1  -orient horiz -relief sunken -command "$c xview"
    scrollbar $f3.hscrol2  -orient horiz -relief sunken -command "$d xview"


    #    pack $f2.hscrol1 -side bottom -fill x
    #    pack $f3.hscrol2 -side bottom -fill x
    pack $f2.vscrol1 -side right -fill y
    pack $f3.vscrol2 -side right -fill y
    pack $c -in $f2.c -fill both -expand yes  -padx 5 -pady 5 
    pack $d -in $f3.c -fill both -expand yes  -padx 5 -pady 5 
    pack $f2.c -in $f2 -fill both -expand yes -after $f2.vscrol1
    pack $f3.c -in $f3 -fill both -expand yes -after $f3.vscrol2
    pack $f2 -side left -fill both -expand yes
    pack $f3 -side left -fill both -expand yes


    bind $c <FocusIn> "$f2.c configure -background grey60"
    bind $c <FocusOut> "$f2.c configure -background grey80"
    bind $c <1> "focus $c"
    bind $d <FocusIn> "$f3.c configure -background grey60"
    bind $d <FocusOut> "$f3.c configure -background grey80"
    bind $d <1> "focus $d"
...
##########################################################################
# proc scroll_canvasy                                                    #
# Scroll canvas $c one point up or down in direction $direction          #
##########################################################################
proc scroll_canvasy {c direction} {
    ...
    # Do not display with left edge of scrollregion to right of
    # left edge of canvas
...
proc reposition_canvas {c tag} {
    ...
    # Tcl8.0 bodge
    move_within_view $c $c_pos
    return
    while {[move_within_view $c $c_pos]==1} {}
}

##########################################################################
# proc move_within_view                                                  #
# If $pos is not within view then scroll one step in appropriate direction
# Returns 1 if scrolling was needed and 0 otherwise
# NB choice of 40 related to the fact that position of top node is 40    #
# If it is less than 40 then cannot scroll to top of scrollbar           #
##########################################################################
...
proc mouse_next_level {c} {
    ...
    #+
    # TREE: experiment_instance navigation nextlevel
    #-
...
proc key_up {c} {
    ...
    # Get list of nodes hanging from parent 
...
proc collapse_rec {node} {
    ...
    # TREE: experiment_instance navigation nextlevel expand_node collapse_node collapse_rec
    # TREE: experiment_instance navigation nextlevel expand_node collapse_node collapse_rec collapse_rec
    #-
...
proc collapse_node {name} { 
    ...
    # TREE: experiment_instance navigation nextlevel expand_node collapse_node
    #-
...
proc shift_node {name y_tot} { 
    ...
    # TREE: experiment_instance navigation nextlevel expand_node shift_node
    #-
...
# ----------------
#
# expand_node - called to expand the next level in the tree
#
# argument - topname , the node to expand.
#
#************************************************************************
...
proc expand_node {topname} {
    ...
    # TREE: experiment_instance navigation nextlevel expand_node
    #-
	...
	} else { set tree($topname,expand) 2 
...
proc itemEnter {e} {
    ...
    # TREE: experiment_instance navigation nextlevel expand_node itemEnter
    #-
...
proc itemLeave {c} {
    ...
    # TREE: experiment_instance navigation nextlevel expand_node itemLeave
    #-
