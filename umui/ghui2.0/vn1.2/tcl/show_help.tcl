# application_help
#   Public procedure for application specific help windows
# Comments
#   All file names should have a .help extension though
# Arguments
#   file_name: File name without path and with or without .help extension
#   title: Title of window
#   path: Path name of file. Defaults to the application help directory

proc application_help {file_name title {path ""}} {
    
    # If not specified assume central help directory
    ...
    # Use file name without "." chars to create toplevel window name
    regsub -all {\.} $file_name _ winname
    set win .$winname\_help
    display_help $path/$file_name $win $title
}    
    
# getHelpText
#   Public procedure to get help text from a particular file and 
#   include any .inc files and substitutions
# Arguments 
#   helpFile: full path name of file name
# Result
#   Returns a formatted string which can be passed to textToWindow

proc getHelpText {helpFile} {

    regsub {/[^/]+$} $helpFile "" path

    # Get help text and remove blank lines from end
    set hf [open $helpFile r]
...
proc show_help {file_name title} {
    ...
    regsub -all {\.} $file_name _ winname
    set win .$winname\_help
    display_help $help_file $win $title
}
...
proc display_help {help_file win title} {
    ...
    set text [getHelpText $help_file]
